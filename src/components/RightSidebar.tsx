...
export const RightSidebar = React.memo(() => {
  return <div className="w-80 bg-gray-900/90 backdrop-blur-md p-6 space-y-6 border-l border-gray-700/50 fixed right-0 top-0 h-screen overflow-hidden z-40 rounded-none">
      {/* Stories Section */}
      <div>
        <h3 className="text-white font-semibold text-lg mb-4">Stories</h3>
        <div className="flex space-x-3">
          {STORIES.map((story, index) => <div key={index} className="flex flex-col items-center">
      ...
      <div>
        ...
        <div className="space-y-3">
          {SUGGESTIONS.map((user, index) => <div key={index} className="flex items-center justify-between">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {RECOMMENDATIONS.map((item, index) => <div key={index} className={`${item.color} p-4 rounded-2xl text-white text-center hover:scale-105 transition-transform cursor-pointer`}>
